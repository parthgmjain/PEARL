...
expTests = testGroup "Expression Tests"
  ...
  , testExp "Var" "x" (Var "x")
  , testExp "Empty list" "'[]" (Const Nil)
  , testExp "List with one element" "'[1]" (Const $ Pair (Num 1) Nil)
  , testExp "List with more elements" "'[1, 2, 3]" (Const $ Pair (Num 1) $ Pair (Num 2) $ Pair (Num 3) Nil)
  , testExp "Nested lists" "'[[], [1, []]]" (Const $ Pair Nil $ Pair (Pair (Num 1) $ Pair Nil Nil) Nil)
